...
This repo does not publish to ArtifactHub, so `artifacthub.io/*` annotations are deliberately absent
from `charts/n8n/Chart.yaml`. Don't reintroduce them.
...
- `CHANGELOG.md` at the repo root must be updated on every release, grouped under the target version
  with `### Added` / `### Changed` / `### Fixed` / `### Removed` headings.
